...
public class CountingGame {
	...
	public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt) {
		int length_rhyme = rhyme.getLength();
		int length_players = players.getLength();
		int index = startAt-1;
		for (int i = 1; i <=length_rhyme; i++) {
			index++;
			if(index==length_players+1)
				index=1;
			System.out.println("Player " + players.getEntry(index) + ": " + rhyme.getEntry(i));
		}
		System.out.println("Removing Player "+players.getEntry(index));
		players.remove(index);
		System.out.println("The Player List is: "+players+"\n");
		if(index==players.getLength()+1)
			index=1;
		return index;
